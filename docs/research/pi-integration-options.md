...
结构化 CLI 封装实际已经接近 “RPC route”。它保留进程隔离，减少把 Pi runtime 拉进 Tauri 进程的风险，同时能拿到足够多的控制面命令。缺点是 Pig 要处理子进程生命周期、JSONL framing、stdout/stderr、版本漂移、崩溃恢复和多 workspace 并发。
...
代价是 Pig 会直接承担 runtime ownership：Node 依赖、Pi 版本锁定、Tauri/Rust 与 Node runtime 边界、extension UI request 映射、project trust、安全边界、sandbox/permissions、崩溃恢复都要设计清楚。SDK 路线不是 “包装 Pi”，而是 “Pig 成为 Pi runtime host”。

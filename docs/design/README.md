...
6. **图标只从 `shared/ui/icons.tsx` 导入。** 统一 `strokeWidth 1.5`；默认 Hugeicons，Usage 的 `chart-pie` 是明确选定的 Lucide 例外。页面不直接导入供应商图标。Sidebar / Header 的动画导出见 [animated-icons.md](animated-icons.md)。

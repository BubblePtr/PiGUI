...
- **图标**：只从 `shared/ui/icons.tsx` 导入（Hugeicons 通过工厂钉 `strokeWidth 1.5`、`currentColor`，导出名按 Lucide 习惯起）。缺图标就在 `icons.tsx` 加一行，`File` 这个名字被故意避开（HMR 会绑到宿主 `File` 构造器）。
  Sidebar / Header 使用独立的 `AnimatedHistory`、`AnimatedChartPie`、`AnimatedPuzzle`、`AnimatedNewChat`、`AnimatedSettings`、`AnimatedSidebar`、`AnimatedPlus`、`AnimatedMoreHorizontal`；Usage 的饼图来自 Lucide，其余来自 Hugeicons。静态导出与 Dock 使用保持不变，具体范围及触发规则见 [animated-icons.md](animated-icons.md)。

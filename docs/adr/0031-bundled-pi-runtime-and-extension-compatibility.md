...
2026-09-12 补充：按用户确认，原生扩展的独立 Node runner 使用机器已有的 Node，由宿主发现并校验；普通 Pi 会话仍无需全局 Node 或 Pi。宿主将固定 SDK 的公开入口与共享 bundle 放到真实文件目录，保持原扩展的调用方式，不要求扩展作者适配 Pace。Node 缺失只使扩展后台能力的可选预检失败，不自动下载运行时。配置与已验证范围见 [系统 Node 扩展运行](../system-node-extensions.md)。

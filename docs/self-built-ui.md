...
| context-usage-meter | `shared/ui/` | 顶部 Session toolbar 的上下文占用指示器(#101;2026-09-01 由 composer `headerContext` 迁至 toolbar,#128——占用是会话运行时状态,与 composer 的 queue/steer 语义无关):**不是从零手搓**——外壳是 Astryx `ProgressBar` + `Tooltip`,自建的是领域语义:`tokens: null`(刚压缩完)显示 `?` 而非假 0、压缩中走 indeterminate、70%/90% 两档告警对齐 Pi CLI footer 自己的阈值。生产形态是 `variant="compact"`(toolbar 行只有一个图标按钮高:短进度条 + 定宽百分比,`tokens/contextWindow` 明细与压缩态走 hover tooltip,同一串文本兼作进度条可访问名,无须 hover 也能被读屏拿到);`default` 形态(`45%/200K` 内联明细)现仅在 /design 页注册。数据链路见下方备注 |

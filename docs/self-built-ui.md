...
| pi-trace-strip | `shared/ui/` | Trace Cockpit 概览带:Input/Model/Tools 三泳道、段粒度、游标竖线、单击选中该泳道块 / 拖拽框选连续段;选区外列与台账行置灰(不过滤)、Steps/Time 双宽度模式;Time 模式模型段用 Pi 记录的模型调用起止真实时长([#108](https://github.com/BubblePtr/PiGUI/issues/108)),缺起止的旧 session 退回轮次间隔估算并以斜纹+弱化标出,估算不伪装成实测 |
...
| context-usage-meter | `shared/ui/` | composer footer 行的上下文占用指示器(#101;2026-09-01 定稿于 footer 行右侧,与 "AI can make mistakes" 提示同行,#128——占用是会话运行时状态,不属于输入控件,进度条形态在 composer header / 顶部 toolbar 两次试放后都嫌喧宾夺主):现在是**一行纯文本** `Context 52%/200K`,只有 Tooltip 是 Astryx,自建的是领域语义:`tokens: null`(刚压缩完)显示 `?` 而非假 0、压缩中直接说 `Compacting…` 而不摆过期份额、70%/90% 两档告警(amber / danger 语义 token)对齐 Pi CLI footer 自己的阈值;精确 `tokens/contextWindow` 走 hover tooltip。queue 模式下提示语消失但 footer 行保留。形态只此一种——进度条形态(default / toolbar compact)已随落点一起删掉,不为 /design 页留死变体。数据链路见下方备注 |
